...
const NAV = [
  ...
  { href: "/baskets", label: "Baskets", icon: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" },
  { href: "/rotation", label: "Rotation", icon: "M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" },
...
function StrategySwitcher() {
  ...
  const options: { value: "BASKET" | "SCALPER"; label: string }[] = [
...
function BotStatusIndicator() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        const p = await fetch(`/api/portfolio?strategy=${strategy}`).then((r) => r.json());
        if (!p || p.error) {
          setStatus(null);
          return;
        }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span style={{ color: status.running ? "var(--green)" : "var(--red)", fontWeight: 600 }}>
        {status.running ? `${strategy} — running` : `${strategy} — paused (${status.reason})`}
      </span>
    </div>
  );
}
